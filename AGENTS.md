...
**推 main 不再触发 CI**：`.github/workflows/build.yml` 的自动触发只有推 `v*` tag
（发布），main 推送与 PR 都不构建（省 GitHub 带宽／额度）。所以"push 成功"不等于
"CI 已验证"——第 1、2 步的本地编译与测试就是唯一的门禁，不得因为"反正 CI 会跑"
而跳过；需要验证流水线本身时用 workflow_dispatch 手动触发（只构建、不发布）。
