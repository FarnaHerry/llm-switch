...
- HuxerUI 源码优先来自 `third_party/huxerui`，CI 固定版本见
  `.github/workflows/build.yml`。当前基线 `445488a` 在 `d1d2daa`（提供
  ApplicationHandle 的 Clipboard/Directories 服务与 TreeView）之上新增
  Windows GUI 子系统启动修复；剪贴板只在 UI 线程通过
...
- 通用卡片使用主题 raised 表面、6pt 圆角和内边距的单层轻量样式，不叠加 SVG
  边框或裁剪层；弹窗可保留规则边框与阴影以保证浮层识别度。
