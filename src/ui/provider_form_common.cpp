...
[[huxerui::composable]] huxerui::View AlternateModelList(
    ...
    const huxerui::ThemeSpec& theme = huxerui::UseTheme();
    // 清单添加：输入框手动填，或从选择源弹层点选直接加入一条；重复提示，
    // 不清空已建清单。主模型字段为空时首个加入的模型顺便回填。
    const auto addToList = [modelList, addModel, fs, toast](std::string id) {
    ...
    for (std::size_t i = 0; i < modelList.Size(); ++i) {
        ...
        row.push_back(
            huxerui::Text(id)
                ...
                .With(huxerui::Grow(1.0F)));
        if (extras.rowTrailing) {
            row.push_back(extras.rowTrailing(i, id));
        }
        row.push_back(
            huxerui::IconButton(app::images::trash, "移除")
                .OnClick([modelList, onRemove = extras.onRemove, i] {
                    modelList.Erase(i);
        ...
        if (extras.panel) {
            items.push_back(extras.panel(i, id));
        }
    }
    items.push_back(huxerui::Row {
