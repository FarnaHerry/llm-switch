...
| cpp-httplib | 0.54.1 | `httplib/httplib.h`（single header，MIT） | 上游 `yhirose/cpp-httplib` v0.54.1；`llmswitch_httplib` INTERFACE 目标导出包含目录，供 llmswitch.router 本地代理服务器（仅监听 127.0.0.1；出站转发走 curl，无 TLS 服务端需求） |
