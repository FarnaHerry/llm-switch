...
- HuxerUI 双通道（顶层 CMakeLists 固定优先级）：显式 `HUXERUI_HOME` 源码 →
  ...
  （git clone 上游，跟主干拉取；当前钉在 `065ab79`，含 ApplicationHandle Clipboard/
...
- **MCP 不支持 claude desktop / pi**：setEnabled/importFromTool 对这两个工具
  抛「该工具暂不支持 MCP 管理」。
- **尚未接入剪贴板 / 打开浏览器交互**：上游 `065ab79` 已提供应用层
...
- `.github/workflows/build.yml`（蓝本 Clash-Flux 同名文件，按其已跑通配方
  ...
  发布门禁，必须完成编译、测试和打包。
- 三个 job 都把 HuxerUI 上游钉在 commit `065ab79`（含 ApplicationHandle
